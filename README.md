 # 👋 Hi, I'm Vasanth Ette

🚀 Java Full Stack Developer | Backend Enthusiast  
🎓 MCA Graduate  
📍 India  

I’m a passionate **Java Full Stack Developer** with a strong interest in **backend development**, building scalable applications, and solving real-world problems using clean and efficient code.

---

## 🚀 About Me
- 💼 Java Full Stack Developer Intern at **CodeBegun** (Jan 2024 – Apr 2024)
- ☕ Strong foundation in **Core Java, J2EE, JDBC**
- 🌐 Hands-on experience with **HTML, CSS, JavaScript**
- 🗄️ Worked extensively with **MySQL**
- 🧩 Familiar with **MVC architecture**
- 💡 Quick learner with strong problem-solving and teamwork skills

---

## 🛠️ Skills

### 💻 Programming Languages
- Java
- JavaScript
- SQL

### 🔧 Backend Technologies
- Core Java
- J2EE
- JDBC
- Spring Boot (Learning)

### 🎨 Frontend Technologies
- HTML
- CSS
- JavaScript

### 🗄️ Database
- MySQL

### 🧰 Tools & Platforms
- Git & GitHub
- Eclipse
- NetBeans
- VS Code

---

## 📂 Projects

### 💼 Online Job Portal
**Tech Stack:** Java, J2EE, JDBC, MySQL  
**Features:**
- User Registration & Login (Job Seeker & Recruiter)
- Job Posting & Job Search
- Apply for Jobs & Application Tracking
- Secure database connectivity
- Role-based access control  

🔗 [View Project](https://github.com/VASANTH-8499)

---

## 🧠 What I’m Currently Learning
- Spring Boot & REST APIs
- Data Structures & Algorithms (DSA)
- Backend System Design Basics
- Microservices Fundamentals

---

## 📊 GitHub Stats
![Vasanth's GitHub Stats](https://github-readme-stats.vercel.app/api?username=VASANTH-8499&show_icons=true&theme=radical)

---

## 📫 Connect With Me
- 🔗 LinkedIn: [Vasanth Ette](https://www.linkedin.com/in/vasanth-ette-576994351)
- 📧 Email: vasanthvasu7447@gmail.com
- 💻 GitHub: [VASANTH-8499](https://github.com/VASANTH-8499)

---

⭐ *Thank you for visiting my GitHub profile!*

